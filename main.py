...
from .main import generate_llr
from utils import Logger, set_seed, get_datasets, get_model, print_args, epoch_adversarial, epoch_adversarial_PGD50
from utils import AutoAttack, Guided_Attack, trades_loss
import wandb

########################## parse arguments ##########################
parser = argparse.ArgumentParser(description='SEAT Adversarial Training')
...
parser.add_argument('--flood', default=0.0, type=float, help='flood level')

########################## attack setting ##########################
adversary_names = ['marg', 'GAT', 'trades']
parser.add_argument('--attack', metavar='attack', default='marg',
...
parser.add_argument('--train_gamma', default=8., type=float, help='step size of attack during training')
...
def main():
    ...
    if args.cyclic:
        lr_scheduler=torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr_max, steps_per_epoch=len(train_loader), epochs=args.epochs)
    else:
        if args.datasets == 'TinyImagenet':
            ...
            print ('default schedule: [60, 80]')
            lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                                milestones=[60, 80], last_epoch=args.start_epoch - 1)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.evaluate, checkpoint['epochs']))
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.wandb:
            wandb.log({"test natural acc": natural_acc, "test robust acc": robust_acc, 'epoch': epoch})
...
def generate_llr(model, input_var, target_var, steps=1):
    adv_images = input_var + ((8.0 / 255.0) * 
                               torch.sign(torch.tensor([0.5]).cuda() 
                               - torch.rand_like(input_var).cuda()).cuda())
    adv_images = torch.clamp(adv_images, 0.0, 1.0)

    for _ in range(steps):
        input_var.requires_grad = True
        out = model(input_var)
        cost = criterion(out, target_var)
        grad = torch.autograd.grad(cost, input_var, retain_graph=False, create_graph=False)[0]
        delta = (adv_images - input_var).flatten().unsqueeze(0)
        grad = grad.flatten().unsqueeze(1)
        temp = torch.mm(delta, grad).item()
        ...
        loss = max_margin_loss(Q_out, target_var) + \
                0.6 * torch.abs(criterion(output, target_var) - criterion(out, target_var) - temp)
        ...
        adv_images = adv_images.detach() + (8.0 / 255.0) * grad.sign()
        delta = torch.clamp(adv_images - input_var, -8.0 / 255.0, 8.0 / 255.0)
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch):
    ...
    Run one train epoch
    """
    global train_robust_acc, train_robust_loss, arr_time, args
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.attack == 'marg':
            input_adv = generate_llr(model, input_var, target_var)
            output = model(input_adv)

            input_var.requires_grad = True
            out = model(input_var)
            cost = criterion(out, target_var)
            grad = torch.autograd.grad(cost, input_var, retain_graph=True, create_graph=False)[0]

            delta = (input_adv - input_var).flatten().unsqueeze(0)
            grad = grad.flatten().unsqueeze(1)
            temp = torch.mm(delta, grad).item()
            reg = torch.abs(criterion(output, target_var) - criterion(out, target_var) - temp)

            loss = (criterion(output, target_var) - args.flood).abs() + args.flood + 0.6 * reg
            ...
            input_adv = input_var + ((4. / 255.0) * torch.sign(
                torch.tensor([0.5]).cuda() - torch.rand_like(input_var).cuda()).cuda())
            ...
            #input_adv = Guided_Attack(model, nn.CrossEntropyLoss(), input_adv, target, eps=10. / 255.0, steps=1,
            #                          P_out=P_out, l2_reg=10, alt=(i % 2))
    ...
    if args.wandb:
        wandb.log({"train robust acc": top1.avg, "train robust loss": losses.avg, "epoch": epoch})
